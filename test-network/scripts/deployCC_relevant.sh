...
checkCommitReadiness_2() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name marbles02_private \
    --collections-config ../chaincode/marbles02_private/collections_config_1.json  \
    --version ${VERSION} --sequence "3" --output json >&log.txt \
    res=$?
    set +x
    let rc=0
    for var in "$@"
...
commitChaincodeDefinition_2() {
  ...
  peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED \
  --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name marbles02_private $PEER_CONN_PARMS --version ${VERSION} \
  --collections-config ../chaincode/marbles02_private/collections_config_1.json --sequence "3" >&log.txt 
  res=$?
  set +x
  cat log.txt
  verifyResult $res "Chaincode definition commit failed on peer0.org${ORG} on channel '$CHANNEL_NAME' failed"
...
chaincodeInvokeInitMarble() {
  ...
  # it using the "-o" option
  set -x
  export MARBLE=$(echo -n "{\"name\":\"marble3\",\"color\":\"red\",\"size\":35,\"owner\":\"tom\",\"price\":29}" | base64 | tr -d \\n)
...
#  checkCommitReadiness_2 1 

#commitChaincodeDefinition_2 1 2 3 4 5 
